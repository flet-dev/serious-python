  ...
  public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
    if (call.method.equals("getPlatformVersion")) {
      ...
      try {
        ...
        // makes R8 outline the call into a synthetic class that it may merge with
        // other API 28+ outlines (e.g. Flutter 3.41's ImageDecoder-based image
        // decoder). Invoking that merged class on API < 28 fails verification with
        // NoClassDefFoundError and crashes the app on launch, because getAppVersion
        // runs on every startup. Guard so older devices use the deprecated int field.
