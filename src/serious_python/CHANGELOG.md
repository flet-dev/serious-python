...
* **iOS/macOS:** the bundled `Python`, `dart_bridge` and stdlib extension XCFrameworks are now signed on both layers — each slice's inner `.framework` as well as the outer `.xcframework`. 4.5.0 signed only the outer bundle, which left IPA receipts reporting `isSecureTimestamp = false`. See `serious_python_darwin` 4.5.1.
